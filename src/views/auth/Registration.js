...
const Registration = () => {
  ...
  const checkFiles = (files) => {
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < files.length; i++) {
        if (files[i]) newFiles.push(files[i]);
        else reject([]);
      }
      resolve(newFiles);
    });
      ...
      try {
        ...
        images.forEach((eachFile) => {
          let storageRef = projectStorage.ref(
            "/Gym/" + email + "/images/" + eachFile.name
          );
          storageRef.put(eachFile).on(
            "state_changed",
            null,
            (err) => console.log(err),
            async () => {
              localPhotos.push(await storageRef.getDownloadURL());
              if (localPhotos.length >= images.length) resolve(localPhotos);
            }
          );
        });
      } catch (error) {
        ...
        docs.forEach((eachFile) => {
          ...
          storageRef.put(eachFile).on(
            ...
            async () => {
              ...
              if (localDocs.length >= docs.length) resolve(localDocs);
            }
          );
        });
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (sameEmail) values.email = email;
      const resultingUser = await auth.createUserWithEmailAndPassword(
        email,
        password
      );
